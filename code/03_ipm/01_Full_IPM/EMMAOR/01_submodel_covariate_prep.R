...
#Climate data is messed up for the full up to 2021 dataset,
##so currently just using data up to 2020

#still working on adult components

#models need:
## new climate data
## add in 2021 when appropriate
## adult occupancy: p-value assessment
...
#need data for ifelse statement for missing years, so
#i'll just have those loop 1:1 - they will be set to the
#mean of all years in the second half of the ifelse statement
nest.start <- c(nest.start[1:4], 1, nest.start[5], 1, nest.start[6])
...
#to deal with missing data
nest.end <- c(nest.end[1:4], 1, nest.end[5], 1, nest.end[6])
...
surveyed <- nest_data2 %>%
  distinct(Year_located) %>%
  mutate(Year_located = factor(Year_located,
                                  levels = c("2014",'2015', '2016',
                                             '2017', '2018','2019',
                                             '2020'))) %>%
  complete(Year_located) %>%
  mutate(surveyed = case_when(Year_located %in% c("2018", "2020") ~ 0,
                              TRUE ~ 1)) %>%
