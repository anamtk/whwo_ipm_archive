...
#Years of survey for EMFWOR:
## Nests: 2015-2021 with no gaps
## Adults: 2015-2021 with no gaps
#2015:2019
#Years with gaps - none
yearIDs <- as.data.frame(cbind(year = c(2015:2021),
                               yearID = 1:length(2015:2021)))

emfwor_df <- final %>%
  left_join(yearIDs, by = "yearID")

emfwor_sum <- emfwor_df %>%
  group_by(year) %>%
...
saveRDS(emfwor_sum, here('data',
                         'ipm_outputs',
                         'EMFWOR_ipm_summary.RDS'))


ggplot(emfwor_sum, aes(x = mu.phi.egg, y = mu.pop.lambda)) +
  geom_point() +
...
ggplot(emfwor_sum, aes(x = mu.phi.nestling, y = mu.pop.lambda)) +
  geom_point() +
...
ggplot(emfwor_sum, aes(x = mu.phi.adult, y = mu.pop.lambda)) +
  geom_point() +
...
ggplot(emfwor_sum, aes(x = year, y = mu.phi.adult)) +
