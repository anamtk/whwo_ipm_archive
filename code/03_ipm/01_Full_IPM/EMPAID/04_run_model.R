...
#Payette is the 3rd forest in the intercept arrays
forest <- 3
#egg number:
...
#Years of survey for EMPAID:
#2012:2019
#Years with gaps - none
yearIDs <- as.data.frame(cbind(year = c(2012:2019),
                               yearID = 1:length(2012:2019)))

empaid_df <- final %>%
  left_join(yearIDs, by = "yearID")

empaid_sum <- empaid_df %>%
  group_by(year) %>%
...
saveRDS(empaid_sum, here('data',
                         'ipm_outputs',
                         'EMPAID_ipm_summary.RDS'))

ggplot(empaid_sum, aes(x = mu.phi.egg, y = mu.pop.lambda)) +
  geom_point() +
...
ggplot(empaid_sum, aes(x = mu.phi.nestling, y = mu.pop.lambda)) +
  geom_point() +
...
ggplot(empaid_sum, aes(x = mu.phi.adult, y = mu.pop.lambda)) +
  geom_point() +
...
ggplot(empaid_sum, aes(x = year, y = mu.pop.lambda)) +
...
ggplot(empaid_sum, aes(x = year, y = mu.phi.adult)) +
