...
occ <- read.csv(here("data",
  ...
  filter(str_detect(Point_ID, "EMPAID" ))
...
covariates <- read.csv(here("data",
  ...
  filter(Point_ID %in% occ$Point_ID)
...
saveRDS(data_list, here('data', 
             ...
             '04_adult_occupancy',
             'jags_input_data',
             'reduced_adult_data_list_empaid.RDS'))
